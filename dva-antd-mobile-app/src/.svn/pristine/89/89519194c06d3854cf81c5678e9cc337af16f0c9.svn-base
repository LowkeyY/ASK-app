...
function HeaderSearch({
  dispatch , defaultSearchText = "请输入搜索内容" , leftContent = {} , rightContent = {}
}) {
  const goSearch = ()=>{
  ...
  const getHtml= (item , right)=> {
    if(!isEmptyObject(item) && item.hasOwnProperty("icon")){
      let html = <Badge dot={item.dot === true}><Icon type={getLocalIcon(item.icon)}/></Badge>;
      if(item.onClick)
        html = <span onClick={item.onClick}>{html}</span>;
      else if(item.to)
        html =  <Link to={item.to}>{html}</Link>;
      return (
  ...
  return (
      <div className={"header-search"}>
        {getHtml(leftContent)}
        <div className={"center"} style={isEmptyObject(leftContent) ? {marginLeft : ".3rem"} : {}}>
