...
function PageContent({location, pagecontent, loading, dispatch, }) {
    ...
    const goBack = () => {
        ...
        handleChange = (e) => {
            dispatch({
                ...
                payload: {
                    currentSelected: e.nativeEvent.selectedSegmentIndex
                }
            })
        },
        handleAtt = () => {
        ...
        getContent = (index = 0) => {
            ...
            case 0: {
                ...
                content.split('_image_').map((_, index) => {
                    if (index % 2 !== 0) {
                        datas.push(<img style={ { maxWidth: '100%' } } src={ require('themes/images/page01.jpeg') } />)
                    }
                    datas.push(<p>
                                 { _ }
                               </p>)
            ...
            case 1: {
                ...
                keys.map((key, index) => {
                    datas.push(<li className={ index === keys.length - 1 ? styles[`${PrefixCls}-props-last-prop`] : '' }>
                                 { key }
                               </li>)
                    datas.push(<li className={ index === keys.length - 1 ? styles[`${PrefixCls}-props-last-value`] : styles[`${PrefixCls}-props-value`] }>
                                 { infos[key] }
                               </li>)
                });
                return (
                            ...
                            return (
                                <List className="my-list" key={ item.id }>
                                  <Item align="top" multipleLine wrap>
                                    <a href="#" className={ styles['discuss-person'] }>
                                      { `${item.name}：` }
                                    </a>
                                    <p className={ styles['discuss-content'] }>
                                      { item.discuss }
                                    </p>
                                    <div>
...
export default connect(({pagecontent, loading}) => ({
    pagecontent,
    loading
}))(PageContent)
