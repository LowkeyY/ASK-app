...
function pdfcontent({location, pdfcontent, loading, dispatch}) {
    ...
    const onScale=(num)=>{
      dispatch({
        type: 'pdfcontent/updateState', payload: {scale: num}
      })
      setTimeout(function () {
        dispatch({
          type: 'pdfcontent/updateState', payload: {isShowScale:false}
        })
      },1000)
    }
    const ShowScale=()=>{
      dispatch({
        type: 'pdfcontent/updateState', payload: {isShowScale:!isShowScale}
      })
    }
