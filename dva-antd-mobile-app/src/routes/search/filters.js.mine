...
function Filters(filters) {
  ...
  const CustomChildren = props => {//DatePicker
   return <div
    ...
    >
      {props.children}
      <span style={{float: 'right', color: '#108ee9'}}>{props.extra}
      {/*<Icon size="xs" style={{verticalAlign: 'middle', marginLeft: '10px'}}*/}
            {/*type={getLocalIcon("/page/delete.svg")}*/}
            {/*onClick={resetDate}*/}
      {/*/>*/}
      </span>
    </div>
      ...
      if (!cntype && items.length) {
        ...
        return (
          <List.Item onClick={handleFilterClick.bind(null, key, items)}
                     arrow={isShow && currentKey == key ? "up" : "down"} extra={getFilterValue(key, title)}
                     ref={el => refs[key] = el}>
            {title}
          </List.Item>
        )
        ...
        if (cntype == "selectuser")
          ...
          return (
            <div>
              <div className={`${PrefixCls}-date-picker-line`}>
                <DatePicker
                  mode="date"
                  title="开始日期"
    ...
    getFilterValue = (key, title) => {
      ...
      return newValue === "" ? `选择${title}(全部)` :
        newValue.startsWith("全部") ? newValue :
          <span>{newValue} <Icon size="xs" style={{verticalAlign: 'middle', margin: '0 10px'}}
                                 onClick={resetValue.bind(null, key)} type={getLocalIcon("/page/delete.svg")}/></span>;
    },
      ...
      <div className={`${PrefixCls}-terms`}>
        ...
        {isShow && currentKey ?
          <SelectMenu {...{
            ...
            values: getValue(),
            multiSelect: false,
            targetRef: refs[currentKey],
            onOk: handleMenuOnOk,
            onCancel: hideSelectMenu
          }}/> : ""}
      </div>
      <WhiteSpace/>
    </div>
