...
class Equipmentinfo extends React.Component {
  ...
  handleItemClick = () =>{
    this.props.dispatch(routerRedux.push({pathname:"/pdfcontent"}))
  };
    ...
    const row = (rowData, sectionID, rowID) => {
        ...
        return (
          <Item className={`${PrefixCls}-row`}
              ...
              wrap
          >
            <div className={"title"}>{obj.title + rowID}</div>
            <Brief>{`${obj.author} - (${obj.date})`}</Brief>
            <div className={"info"}>
              <p>位置:<span>{obj.filePath}</span></p>
              <p>大小:<span>{obj.fileSize}</span></p>
            </div>
          </Item>
        );
    };
