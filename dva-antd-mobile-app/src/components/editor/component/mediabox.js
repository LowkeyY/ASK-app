...
const SmilmgName=emotion.SmilmgName.tab5[0]
  return(
     <div className={styles['RichEditor-emjoy-box']}>
       <ul>
         {
          arr.map((data,i)=>{
           return <li><img src={`${BASE_URL}face/${SmilmgName}${i<10?`0${i}`:i}.gif`}/></li>
          })
...
const MediaControls=(props)=>{
  ...
  return (
    <div className={styles['RichEditor-controls']}>
      ...
      /><Icon type={getLocalIcon('/editor/sendimg.svg')} size="xxs"/></span>
      ...
      <span className={styles['RichEditor-controls-item']}><EmjoyBox/><Icon type={getLocalIcon('/editor/emjoy.svg')} size="xxs"/></span>
    </div>
  )
}
