...
export default function formsubmit(url, params, files, isCNDefined = false) {
    ...
    if (isOriginal) {
        ...
        const newFiles = {},
            promiseFiles = [],
            promiseArray = Object.keys(files).map((key) => {
                const f = files[key],
                    {size = 0, type = "image/jpeg", name ="img_" + (cnGlobalIndex++) + ".jpg"} = f;
                return transformFileToDataUrl(f).then(({data, orientation = 1}) => {
                    promiseFiles.push(compressDataUrlToFile(data).then((img) => {
                        compressFile(img, {
                            size,
                            type,
                            name
        ...
        return Promise.all(promiseArray).then(() => {
            return Promise.all(promiseFiles).then(() => {
